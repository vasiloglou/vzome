...
- Phase 44 Plan 03 — NARR-03: Enriched Section 5 of `developers-docs/guided-design-tutorial.md` with a paragraph each for `energy_proxy_ev_per_atom` (estimated cohesive energy per atom, more negative = more stable) and `min_distance_proxy` (minimum normalized intersite distance, below 0.7 = geometry error), a "How the shortlist works" paragraph explaining passed_count (20/30) and shortlisted_count (4) as a compute-cost bottleneck, and a "What the numbers mean" blockquote callout showing the first candidate's values. NARR-04: Enriched Section 6 with a "Why all gates are False" paragraph framing the mock-backend sentinel values as the correct honest early-stage behavior (not a pipeline failure), followed by a "Release gate checklist" blockquote with all four gates listed and the all-False interpretation explained. NARR-05: Enriched Sections 9.1, 9.2, and 9.3 with explain-then-command-then-annotate paragraphs for `llm-generate`, `llm-evaluate`, `llm-translate`, `llm-translated-benchmark-freeze`, `llm-register-external-target`, and `llm-external-benchmark`, plus a "What the chemistry switch means" paragraph in Section 9.2 explaining the Al-Cu-Fe context switch as fixture-availability rather than a new authority chain. Four condensed explanation cells were added to `notebooks/guided_design_tutorial.ipynb`: "What the screening numbers mean" after cell b1aada31, "What the validation gates mean" after cell 4509cd69, "What the LLM companion lane does" replacing cell 1119a5de, and "What the translation and benchmark branch does" after cell 123812e6, each with a link to the corresponding full markdown section.
- Phase 44 Plan 02 — NARR-01: Added the design-origin narrative block ("## About This Design") to `developers-docs/guided-design-tutorial.md` between Section 1 and Section 2. The block explains what a Tsai-type icosahedral quasicrystal approximant is (concentric polyhedral shells that tile 3D space), why the Sc-Zn system was chosen (IUCrJ 2016 Takakura et al., PMC4937780, best-characterized Tsai-type structure), and the bridge concept (`.zomic` file connects human geometric intuition to machine-readable candidate generation). NARR-02: Added subsection 2.1 "Reading the Zomic Design Source" inside Section 2 with a label glossary table mapping `pent`, `frustum`, and `joint` to their physical parts, preferred species, and site counts, followed by four annotated Zomic code blocks (frame declaration, pentagonal ring, frustum connectors, joint sites) each with inline `# <-` explanation comments and a physical result sentence. Condensed versions of both additions were added as two new markdown cells in the notebook: the design narrative cell (id=a8d3f4b1) before cell b7279499, and the Zomic annotation cell (id=c9e2b5d7) after cell 21898478, both with links to the markdown for the full account. No existing section headings were renumbered.
