...
| Date | Change | Details |
|------|--------|---------|
| 2026-04-15 | Phase 42 extensive guided tutorial expansion | Reframed `developers-docs/guided-design-tutorial.md` around the Sc-Zn deterministic spine, added the explicit branch map, moved the repo-owned preview path into the main walkthrough, replaced the old desktop-only visualization handoff with a clearer repo-preview versus desktop-vZome boundary, and expanded the same-system plus translation/external benchmark branches into concrete artifact-backed operator guidance |
