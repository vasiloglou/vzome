...
def test_cli_help_lists_checkpoint_lifecycle_workflow_commands() -> None:
    runner = CliRunner()

    result = runner.invoke(app, ["--help"])

    assert result.exit_code == 0
    assert "llm-register-checkpoint" in result.stdout
    ...
    assert "llm-serving-benchmark" in result.stdout


def test_cli_help_lists_translation_workflow_commands() -> None:
    runner = CliRunner()
