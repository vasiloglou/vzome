...
def _write_stage_artifacts(
    ...
    *,
    ...
    write_json_object(
        {"stage": "screen", "created_at_utc": "2026-04-04T19:00:00Z", "source_lineage": lineage},
    ...
    if include_report:
        write_json_object(
            {"stage": "report", "created_at_utc": "2026-04-04T19:15:00Z", "source_lineage": lineage},
...
def test_cli_llm_compare_with_prior_launch(tmp_path: Path, monkeypatch) -> None:
    runner = CliRunner()
    prior_launch = _write_launch_bundle(
        tmp_path,
        launch_id="launch-001",
        created_at_utc="2026-04-04T18:00:00Z",
    )
    _write_stage_artifacts(tmp_path, launch_id="launch-001", launch_summary_path=prior_launch)
    current_launch = _write_launch_bundle(
        tmp_path,
    ...
    result = runner.invoke(
        app,
        ["llm-compare", "--launch-summary", str(current_launch)],
    )
    ...
    assert llm_campaign_comparison_path(
        "campaign-001",
...
def test_cli_llm_compare_without_prior_launch_reports_missing_baseline(
    tmp_path: Path,
    monkeypatch,
) -> None:
    runner = CliRunner()
    ...
    assert result.exit_code == 0
    assert "Prior launch baseline: none" in result.stdout
    assert "Current outcome missing metrics:" in result.stdout
