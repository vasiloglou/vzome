...
- [x] **DATA-01**: The platform can ingest at least one QC-specific open source
  dataset and at least three general materials databases into a canonical raw
  staging format.
- [x] **DATA-02**: Every ingested record carries source provenance, source
  version/snapshot metadata, license metadata, and an immutable local record ID.
- [x] **DATA-03**: The ingestion layer supports both direct source-specific
  adapters and an OPTIMADE-based adapter path for compatible databases.
- [x] **DATA-04**: The ingestion layer emits QA metrics for duplicates, missing
...
- [x] **OPS-03**: The system keeps the no-DFT boundary explicit in required
  execution paths.
- [x] **OPS-04**: The roadmap preserves compatibility with the current
...
*Last updated: 2026-04-04 after Phase 09 execution completed all v1 requirements across data, pipeline, and LLM tracks*
