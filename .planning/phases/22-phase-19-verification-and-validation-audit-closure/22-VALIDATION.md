...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| 22-01-01 | 01 | 1 | LLM-13, LLM-14, OPS-08 | focused regression | `cd materials-discovery && uv run pytest tests/test_llm_launch_schema.py tests/test_llm_runtime.py tests/test_llm_generate_core.py tests/test_llm_generate_cli.py tests/test_llm_launch_core.py tests/test_llm_launch_cli.py tests/test_llm_replay_core.py tests/test_cli.py -x -v` | ✅ | ⬜ pending |
| 22-01-02 | 01 | 1 | LLM-13, LLM-14, OPS-08 | doc consistency | `git diff --check` | ✅ | ⬜ pending |
| 22-02-01 | 02 | 2 | LLM-13, LLM-14 | proof matrix | `git diff --check` | ✅ | ⬜ pending |
| 22-02-02 | 02 | 2 | OPS-08 | proof matrix | `git diff --check` | ✅ | ⬜ pending |
| 22-03-01 | 03 | 3 | LLM-13, LLM-14, OPS-08 | traceability | `git diff --check` | ✅ | ⬜ pending |
| 22-03-02 | 03 | 3 | LLM-13, LLM-14, OPS-08 | state + self-closeout | `git diff --check` | ✅ | ⬜ pending |
...
- [ ] `19-VERIFICATION.md` must exist by the end of execution
- [ ] `19-VALIDATION.md` must move from `status: draft` to an audit-ready state
- [ ] `REQUIREMENTS.md` should only move `LLM-13`, `LLM-14`, and `OPS-08` back
...
- [ ] Phase 22 should finalize its own validation state and leave behind a
